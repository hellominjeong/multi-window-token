...
from . import merge
from tomesd.merge import create_windows, calculate_similarity, merge_windows, reconstruct_from_windows
...
def compute_merge(x: torch.Tensor, tome_info: Dict[str, Any]) -> Tuple[Callable, ...]:
    ...
    if downsample <= args["max_downsample"]:
        ...
        x = merge.window_based_soft_matching(
            input_tensor=x,
            w=w, 
            h=h,
            r=r,
        ...
        m, u = (lambda x: x, lambda x: x)  # Modify as needed
    else:
        m, u = (lambda x: x, lambda x: x)
